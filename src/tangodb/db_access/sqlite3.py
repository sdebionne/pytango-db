...
class SqlDatabase(object):
    ...
    def __init__(
        self, name, db_path="tango_database.db", history_depth=10, fire_to_starter=True
    ):
        self._db_api = None
        self._db_conn = None
        self.name = name
    ...
    def db_conn(self):
        if self._db_conn is None:
            self._db_conn = self.db_api.connect(self.db_path)
    ...
    def info(self):
        ...
        result = []
        # db name
        info_str = "TANGO Database " + self.db_path
...
def get_db(personal_name="2", db_path="tango_database.db"):
    #    return Executor.submit(Sqlite3Database).result()
    return Sqlite3Database(personal_name, db_path)
